...
constexpr float LIGHT_RADIUS = 1.5f;
constexpr glm::vec3 LIGHT_ORIGIN(0.0f);
...
static glm::vec3 lineSphereIntersection(const glm::vec3 &lineOrigin, const glm::vec3 &lineDirection,
                                        const glm::vec3 &sphereOrigin, float sphereRadius) {
    const glm::vec3 sphereOriginToLineOrigin    = lineOrigin - sphereOrigin;
    const float repeatedDot                     = glm::dot(lineDirection, sphereOriginToLineOrigin);
    const float delta                           = std::pow(repeatedDot, 2) - 
                                                  (std::pow(glm::length(lineOrigin - sphereOrigin), 2) - std::pow(sphereRadius, 2));
    const float directionMultiplier             = repeatedDot + std::sqrt(delta); // Keep only positive solution
    return lineOrigin + (directionMultiplier * lineDirection);
}
...
static glm::vec3 userInteractionSphere(const glm::vec3& selectedPos, const glm::vec3& camPos) {
    ...
    return lineSphereIntersection(selectedPos, camPos - selectedPos, LIGHT_ORIGIN, LIGHT_RADIUS);
}
...
static glm::vec3 userInteractionShadow(const glm::vec3& selectedPos, const glm::vec3& selectedNormal, const glm::vec3& lightPos) {
    ...
    // Acquire direction orthogonal to normal and compute intersection of light sphere with line having this direction and selectedPos as origin
    const float orthogonalX = selectedNormal.x > 0.0f ? 1.0f : -1.0f;
    const float orthogonalY = selectedNormal.y > 0.0f ? 1.0f : -1.0f;
    const float orthogonalZ = ( -(orthogonalX * selectedNormal.x) - (orthogonalY * selectedNormal.y)) / selectedNormal.z;
    const glm::vec3 orthogonalToNormal = glm::normalize(glm::vec3(orthogonalX, orthogonalY, orthogonalZ));
    return lineSphereIntersection(selectedPos, orthogonalToNormal, LIGHT_ORIGIN, LIGHT_RADIUS);
}
...
static glm::vec3 userInteractionSpecular(const glm::vec3& selectedPos, const glm::vec3& selectedNormal, const glm::vec3& lightPos, const glm::vec3& cameraPos) {
    ...
    const glm::vec3 normalNorm      = glm::normalize(selectedNormal);
    const glm::vec3 reverseViewNorm = glm::normalize(selectedPos - cameraPos);

    // Compute light position with reflection vector equal to view vector (https://math.stackexchange.com/a/13263)
    return reverseViewNorm - (2.0f * glm::dot(reverseViewNorm, normalNorm) * normalNorm);
}
...
int main(int argc, char** argv)
{
    ...
    const Mesh mesh = loadMesh(argc == 2 ? argv[1] : utils::RESOURCES_PATH / "dragon.obj")[0];
    struct {
        // Diffuse (Lambert)
        glm::vec3 kd { 0.5f };
    ...
    window.registerKeyCallback([&](int key, int /* scancode */, int action, int /* mods */) {
        ...
        case GLFW_KEY_R: {
            if (shiftPressed) { lights[selectedLight].color.r -= 0.1f; } // Decrease diffuse Kd coeffcient in the red channel by 0.1
            else { lights[selectedLight].color.r += 0.1f; } // Increase diffuse Kd coeffcient in the red channel by 0.1
            lights[selectedLight].color.r = std::clamp(lights[selectedLight].color.r, 0.0f, 1.0f);
            return;
        }
        case GLFW_KEY_G: {
            if (shiftPressed) { lights[selectedLight].color.g -= 0.1f; } // Decrease diffuse Kd coeffcient in the green channel by 0.1
            else { lights[selectedLight].color.g += 0.1f; } // Increase diffuse Kd coeffcient in the green channel by 0.1
            lights[selectedLight].color.g = std::clamp(lights[selectedLight].color.g, 0.0f, 1.0f);
            return;
        }
        case GLFW_KEY_B: {
            if (shiftPressed) { lights[selectedLight].color.b -= 0.1f; } // Decrease diffuse Kd coeffcient in the blue channel by 0.1
            else { lights[selectedLight].color.b += 0.1f; } // Increase diffuse Kd coeffcient in the blue channel by 0.1
            lights[selectedLight].color.b = std::clamp(lights[selectedLight].color.b, 0.0f, 1.0f);
            return;
        }
        default:
            return;
    ...
    const Shader lightShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "light_vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "light_frag.glsl").build();
    const Shader debugShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "debug_frag.glsl").build();
    const Shader lambertShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "lambert_frag.glsl").build();
    const Shader phongShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "phong_frag.glsl").build();
    const Shader blinnPhongShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "blinn_phong_frag.glsl").build();
    const Shader toonDiffuseShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "toon_diffuse_frag.glsl").build();
    const Shader toonSpecularShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "toon_specular_frag.glsl").build();
    const Shader xToonShader = ShaderBuilder().addStage(GL_VERTEX_SHADER, utils::SHADERS_PATH / "vertex.glsl").addStage(GL_FRAGMENT_SHADER, utils::SHADERS_PATH / "xtoon_frag.glsl").build();
    ...
    stbi_uc* pixels = stbi_load((utils::RESOURCES_PATH / "toon_map.png").string().c_str(), &width, &height, &sourceNumChannels, STBI_rgb);
            ...
            for (const Light& light : lights) {
                ...
                if (!renderedSomething) {
                    if (toonxLighting) {
                        ...
                        glUniform1i(7, 0); // Change 2 to the uniform index that you want to use.
                        render();
                    } else {
                        if (toonLightingDiffuse) {
                        ...
                        if (toonLightingSpecular) {
                            ...
                            glUniform1f(5, shadingData.shininess);
                            glUniform1f(6, shadingData.toonSpecularThreshold);
                            render();
                        }
                    }
                }
                if (!renderedSomething) {
                    ...
                    if (phongSpecularLighting || blinnPhongSpecularLighting) {
                        if (phongSpecularLighting) { phongShader.bind(); }
                        else { blinnPhongShader.bind(); }
...
static std::optional<glm::vec3> getWorldPositionOfPixel(const Trackball& trackball, const glm::vec2& pixel)
{
    ...
    const glm::vec4 viewport { 0, 0, utils::WIDTH, utils::HEIGHT };
