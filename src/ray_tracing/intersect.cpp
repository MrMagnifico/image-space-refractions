...
bool intersectRayWithPlane(const Plane& plane, Ray& ray) {
    ...
    if (utils::zeroWithinEpsilon(dirNormDot)) { return false; } // Scenario where line lies on plane considered not an intersection
...
bool pointInTriangle(const glm::vec3& v0, const glm::vec3& v1, const glm::vec3& v2, const glm::vec3& n, const glm::vec3& p) {
    ...
    return utils::inRangeInclusive(barycentricCoords[0], 0.0f, 1.0f) &&
           utils::inRangeInclusive(barycentricCoords[1], 0.0f, 1.0f) &&
           utils::inRangeInclusive(barycentricCoords[2], 0.0f, 1.0f);
}
...
bool intersectRayWithShape(const Sphere& sphere, Ray& ray, HitInfo& hitInfo) {
    ...
    else if (utils::zeroWithinEpsilon(discriminant))    { tNew = -B / (2.0f * A); }
